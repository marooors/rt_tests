...
def fill_missing_intervals(data, min_time, max_time):
    filled_data = []
    current_time = min_time
    data_dict = {time: count for time, count in data}

    while current_time <= max_time:
        count = data_dict.get(current_time, 0)
        filled_data.append((current_time, count))
        current_time += 1
...
def plot_histogram(data, filename, plot_name):
    ...
    min_time = min(d[0] for d in data)
    max_time = max(d[0] for d in data)

    filled_data = fill_missing_intervals(data, min_time, max_time)

    times = [d[0] for d in filled_data]
    counts = [d[1] for d in filled_data]
    ...
    bars = plt.bar(range(len(times)), counts, color="red", edgecolor="red")
